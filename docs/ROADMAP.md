...
- [x] `web/fortune.py` API + `data/fortune/runs.db` store — run lifecycle (`/api/fortune/new`, single `/action` verb endpoint, `/deploy`, `/battle` shaped like `/api/battle`, `/spin`, `/leaderboard`), in-memory sessions + sqlite Book of Ages; `tests/test_fortune_api.py` (10)
- [x] `/supertemporal` page — lobby (books/seed/handle), Cross-Trade shop (stable + stock cards, item shelf, freeze/reroll, train/sell/give with targeting), foresight queue + enemy bill, pointer-drag deployment on the SVG board (gilt zone, client pre-check + server validation), auto-replay from the event stream (same pure fold as the Pit), three-ring gold wheel animated to the server's stops, Book of Ages table; gilt/silver page accents, lacquer red reserved for the wheel
