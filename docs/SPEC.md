...
- 18.8 **The Supertemporal Arena** (auto-battler page; themed on the Fortune's Wheel casino, Sigil — Shemeshka is the purveyor). A roguelite run: assemble a stable of up to **5 fighting monsters plus 1 standby stall** (the standby sits out the battle; any two stalls can trade places) through a shop, battle auto-generated enemy compositions, survive as far as possible before **3 losses** end the run. Monsters-vs-monsters (mode named after the Platinum Room's arena of *Turn of Fortune's Wheel*). `[S12e]`
  ...
  - 18.8.7 **Training (elites).** Two owned copies of the same monster can be **combined**: the survivor becomes an *elite* (★ per training level) with **+1 AC and +1 damage per level** (the damage lands on the first damage component of each attack); levels sum on merge, items transfer up to the 3-item cap (excess lost), invested gold accumulates for sell-back.
