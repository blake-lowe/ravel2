...
- 18.8 **The Supertemporal Arena** (auto-battler page; themed on the Fortune's Wheel casino, Sigil — Shemeshka is the purveyor). A roguelite run: assemble a stable of up to **5 fighting monsters plus 1 standby stall** (the standby sits out the battle; any two stalls can trade places) through a shop, battle auto-generated enemy compositions, survive as far as possible before **3 losses** end the run. Monsters-vs-monsters (mode named after the Platinum Room's arena of *Turn of Fortune's Wheel*). `[S12e]`
  ...
  - 18.8.3 **Currency.** Standard D&D coinage, stored in cp: 1 gp = 10 sp = 100 cp; every price and balance renders as gp/sp/cp change. The purse **resets to 10 gp at each shop phase** — unspent coin is forfeit to the house; gold won on the wheel lands on top of the fresh purse.
  - 18.8.4 **Shop.** 5 monster slots + 2 item slots per roll. Reroll costs **5 sp**; per-slot **freeze** persists a slot through rerolls and into the next shop. Monster price = **3 gp × playtested CR ÷ shop tier**: `max(5, round(300 × best_cr / tier))` cp — weaker stock gets cheaper as the tier climbs, and the adjusted-CR ledger *is* the price tag (unrated monsters price at book CR). Item prices by rarity: common 2 gp, uncommon 4 gp, rare 6 gp, with per-item overrides (`ArenaItem.price_cp`): the Manual of Gainful Exercise is a flat 5 gp and the resistance wards 3 gp (near-zero generic value; they are counter-buys after divining a typed foe). Item stats are balance-tested by mirror-match uplift (item on one side of an otherwise identical battle): commons sit ≈ +7–12%, uncommons ≈ +15–22%, rares ≈ +22–40%. Shelf rarity odds rise with the stock tier: uncommon climbs from 25% (+5%/tier, capped 50%), and rare stock reaches the shelf from tier 3 (5% per tier past 2, capped 25%). New stock **deals in** with a reveal (typewriter text, a top-left art swipe, then the border draws itself clockwise from opposite corners; also on fusion results; respects `prefers-reduced-motion`); prices ink by denomination (≥ 1 gp a still gold sheen, ≥ 1 sp dark gray, under 1 sp copper), and stock priced **over 3 gp** (rated above the stock tier) bears the house's lacquer **seal watermarked** behind its figures. Further table-feel: the purse pulses on change (gold gain / lacquer spend), a lost life burns its chip red with a shake (a regained one rekindles), the verdict stamps in with a notary's slant, the wheel's long tail passes the pointer once before settling, and tokens drop onto the sands with a bounce when the battle opens. Selling a monster fetches **half its price as if bought at the current tier** (`price_cp(entry, tier) // 2`) — training, items, and fusions add nothing to the sale; items are never refunded.
  - 18.8.5 **CR cap ladder.** Shop and enemy pools admit only CR ≤ cap; `cap(round) = 1 + (round − 1) // 2` (cap rises by 1 after every 2 battles). Shop rolls weight toward the top of the unlocked band (`w = 1/(1 + max(0, cap − cr))`); owned monsters roll at full weight (duplicates feed training, 18.8.7).
  - 18.8.6 **Items (kit boons).** Buying an item attaches it to one owned monster (max **3 items per monster**, permanent, lost on sell). An item is a pure stat-block transform applied at battle build time (`apply_kit`): flat deltas to AC, max HP, attack to-hit, attack damage, speed — plus **damage resistances** (uncommon wards), **damage immunities** (rare), and **favored-foe attack advantage** against a creature type or an alignment ("evil"/"good"), modeled on classic D&D magic items (Ring of Warmth, Brooch of Shielding, Periapt of Proof against Poison, Efreeti Chain, Dragon Slayer, Giant Slayer, Mace of Disruption, the Talismans). The advantage hook is a `MonsterDef` field (`adv_against_types`/`adv_against_aligns`) consulted in `resolve_attack`; alignment matches both 5e.tools codes ("C E") and prose. Catalog is data in `ravel/fortune.py` (4 common / 7 uncommon / 11 rare, planar-flavored blurbs).
  - 18.8.7 **Training (elites), the manual, and overtier stock.** Two owned copies of the same monster can be **combined**: the survivor becomes an *elite* (★ per training level) with **+1 AC and +1 damage per level** (the damage lands on the first damage component of each attack); levels sum on merge, items transfer up to the 3-item cap (excess lost). Training is capped at **3 stars** — no merge, purchase, or manual may pass it. The **Manual of Gainful Exercise** (uncommon shelf and wheel pool, flat **5 gp**) trains *any* creature one level instead of being carried. **Overtier offerings:** reaching 3 stars appends a bonus sale slot holding a **random creature from the band above the CR cap** (cap < CR ≤ cap + 1; nearest available when the band is empty), priced by the standard formula and azure-sheened in the UI. Rerolls never touch an overtier slot; crossing into a **new night it survives only when frozen (❄)**, like any other slot. **Type sets:** owning **5 creatures of one creature type** (the standby counts) also summons an overtier offering — **of that same type** — once per type per run; the shop header shows the closest unclaimed set ("3/5 aberrations"). **Fuse:** any two owned creatures sharing a **creature type or an alignment** combine into one creature of **CR = 1 + the average of their CRs, capped by the stock tier** — drawn at random from the shared group's highest CR band at or under that target, keeping the shared kind (or creed; kind outranks creed when both match). Neither items nor training survive the fusion (unburden first). The fuse menu shows the formula, offers choose buttons only on valid partners, and can be cancelled.
  - 18.8.8 **Fortune's Wheel.** Every battle **won** grants one spin of a three-ring wheel (gold, red accents). Outer ring: 3/10 no prize, 5/10 common prize, **2/10 advance** to the middle ring. Middle ring: 1/10 no prize, 7/10 uncommon prize, **2/10 advance** to the center. Center ring: always a rare prize. The two advance stars sit on opposite sides of each ring and the no-prize sectors are spread apart. Pools — common: {2 gp, 1 gp 5 sp, random common item}; uncommon: {5 gp, random uncommon item}; rare: {random rare item, +1 life (cap 3; 10 gp if already at cap), 10 gp}. All outcomes drawn from the run RNG server-side; the client only animates to the returned stops.
  - 18.8.9 **Enemy generation.** Seeded from the run: team size `min(5, 2 + (round + 1) // 2)`, filled by weighted sampling from the selected books under the CR cap against an XP budget `size × XP(cap) × 0.75 × U[0.85, 1.15]` (adjusted XP where rated). Duplicates allowed. Each squad shares **one cohesion trait**, drawn per round: half the nights band by **creature type**, half by **alignment** (5e.tools codes and prose both normalize to the same words). The group is picked weighted by how much of the unlocked band it fills, among groups with at least one affordable member (the whole band is the fallback), and the squad is built from that group's pool alone — the house sends cohorts, not menageries. **Boss nights:** every other stock-tier increase (rounds 3, 7, 11, …) fields a **single monster** instead — drawn from the selected books with the CR cap waived, XP-matched to **1.5×** the whole team budget (a lone boss pays an action-economy premium; pick within [0.65, 1.15]×, else nearest) — and is announced in the foresight queue.
  ...
  - 18.8.13 **Persistence & leaderboard ("the Book of Aeons").** Shown on the mode's landing view only, never during a run. When the third life is spent the player is prompted for up to 3 initials (arcade-style), carved beside the run with its date. Active runs live in server memory; finished runs persist to sqlite (`data/fortune/runs.db`): seed, books, victories, rounds survived, final stable (with items/elites), timestamp. Endpoints serve a leaderboard (top victories) with each run's final composition; the top three entries wear gold, silver, and bronze plates. Score = battles won.
